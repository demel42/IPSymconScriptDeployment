...
if (function_exists('GetLocalConfig') == false) {
    function GetLocalConfig(string $ident)
    {
        $script_map = [
            'GLOBAL_HELPER' => xxxxx, // z.B. Hilfsfunktionen
        ];
        $object_map = [
        ];
        $guid2inst_map = [
        ...
        if ($ret === false) {
            IPS_LogMessage(__FUNCTION__, 'configuration with keyword "' . $ident . '" not found');
        }
        return $ret;
    }
}
